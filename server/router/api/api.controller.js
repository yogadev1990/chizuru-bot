...
import { ButtonResponse, ListResponse, VipGrup } from "../../database/db/messageRespon.db.js";
...
class ControllerApi extends ConnectionSession {
	...
	async vip(req, res) {
		try {
			let { sessions, target, groupid } = req.body;
			if (!target || !groupid || !sessions) {
				return res.send({ status: 400, message: "Input All Data!" });
			}
			await new VipGrup().Vipstart(groupid);
			sessions = sessions.includes("(") ? sessions.split(" (")[0] : sessions;
			const { client, toTarget } = await this.clientValidator(req, res, sessions, target);
			if (!client || !toTarget) return;
			await new Client(client, toTarget).sendText("Langganan Chizuru Bot Berhasil ditambahkan selama 30 Hari. Silakan cek waktu tersisa Chizu bot di grup kamu");
			return res.send({ status: 200, message: `Success Adding Subscription` });
		} catch (error) {
			console.log(error);
			return res.send({ status: 500, message: "Internal Server Error" });
		}
